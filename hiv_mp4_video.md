...
每个分段的开始位置可以从`indexXX.bin`目录文件中[找到](./index.md#jump_s4_video)。但要注意，目录文件中记录的是音视频数据开始的位置（也即第5段开始的位置）。若要得到分段开始位置，需要将目录文件中记录的每一分段开始位置**减**去`0x40000`。
...
本节开始位置固定为相对分段开始后`0x0`，观察知其长度为`0x10000`。
...
| 0x0C | 4 | uint32 | 本片段音视频数据长度<sup>2</sup> |
...
本节开始位置固定为相对分段开始后`0x10000`，观察知其长度为`0x10000`。

本节包括一个不符合标准的 private_stream_2 PES packet。如果某一片段包含了紧急录像，则此处记录了片段内紧急录像的信息。
...
| body | 0x20 | 0x10 | 片段内第1段紧急录像信息 |
|  | 0x30 | 0x10 | 片段内第2段紧急录像信息 |
|  | ... | 0x10 | ... |
...
如果某一片段视频里面没有紧急录像，则此处的长度应为`0x00`，后面数据体里面也全为`0x00`。
...
本节开始位置固定为相对分段开始后`0x20000`，观察知其长度为`0x1E000`。
...
本节开始位置固定为相对分段开始后`0x3E0000`，观察知其长度为`0x2000`。
...
本节开始位置固定为相对分段开始后`0x400000`，内容为视频、录音、GPS、加速度计、红绿灯识别、前车起步识别等信息。其长度可变，由[第1节](#第1节)的header内信息确定。
...
最后是private_stream_1数据包，其包括内容很多，且数据为自定义数据。数据种类不同，出现频率也不同。

### program_stream_map video audio

对于program_stream_map、视频和音频的PES_packet结构，请参阅相关公开的标准文件，这里仅讨论行车记录仪自定义的private_stream_1的结构。
...
<sup>1</sup> 此处的PES包长度的含义是长度之后的一字节开始（含），到整个PES包的最后一字节（含）的长度。

<sup>2</sup> PES_header_data_length指的是PTS和stuffing_byte的长度之和。
