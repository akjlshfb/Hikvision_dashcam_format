...
| 0x0802 | 0x0001 | [红绿灯、前车起步、GPS、加速度计等](#jump_misc_1) |
| 0x0009 | 0x0001 | [红绿灯、加速度计等](#jump_misc_2) |
...
| 偏移量 | 字节数 | 类型 | 猜测含义 |
| ---- | ---- | ---- | ---- |
| 0x0 | 4 | uint32 | 本个PES包的[内部时间戳](#jump_timestamp) |
| 0x4 | 4 | NA | 不明 |
...
## <span id="jump_misc_2">pkt_id=0x0009 sub_pkt_id=0x0001 多种数据</span>

这一类数据包与[上一类数据包](#jump_misc_1)结构非常相似。很多数据完全相同。
...
part_3长度总是`part_3_len - 0x650`。这里与上一节不同，上一节中减去的数字是`0x64C`。`part_3_len`定义见下表：
...
其他part的含义都和[上一节](#jump_misc_1)相同，请参照上一节内容。
